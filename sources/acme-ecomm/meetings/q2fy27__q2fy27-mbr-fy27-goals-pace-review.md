...
That's a week-over-week drop of roughly 38bps (3.24% down to 2.86%). App session share jumped from 28.0% to 37.6% over that same week, and app converts structurally lower than web in our current funnel, so some of that move is mix. Wei's team still needs to run the full shift-share split before we put a mix-vs-rate number in the deck.
...
**owen.faust:** Checkout Simplify (`exp_2214`, ran Feb 16 to Mar 30) shipped to 100% on April 6. Just a reminder for the audit trail: its full-window readout showed a +2.1% lift, but that was confounded because Nav Refresh launched into both experiment arms on March 1, and Nav Refresh's own holdback read an independent +1.3% sitewide. So the checkout win is real on some basis smaller than +2.1%, even if we shipped on the headline number.
...
**victor.okonkwo:** Worth looking at side by side with Resold, Deborah. Remember back in Q1 when we looked at the abandoned "Style Conversion Recovery Plan" draft from Confluence? At the time, people were worried Style was losing steam and suggested shifting Trust & Safety headcount away from Collectibles to fix Style. Our April Confluence review put the two categories next to each other instead of looking at Style alone: Resold's apparel and style-adjacent category share climbed from 51% to 62% YoY, and Resold overall is running at +90.9% YoY against Style's +6.1%. We paused the headcount ask pending a fuller read rather than act on the Style number in isolation.
...
Deflected CSAT moved from 3.70 down to 3.42 in Q1 and has partially rebounded to **3.55** this quarter (agent-assisted CSAT remains steady and healthy at 4.31), over the same window the Ontario situation played out. Deflection itself has been rising since before "Ask Acme v2" launched (September 15, 2025) and through Aisha's recent bot handoff experiments. I'll leave it to whoever's building the attribution story to decide how much of each series explains the other.
...
Pickup on-time rates are virtually flawless at **99.6%**, and pickup mix has surged 8 full percentage points since last year, following the Pickup Perks BOPIS/curbside discount campaign tara.oduya's team launched back on January 15. Meanwhile, ship-to-home—our hardest, most complex channel—actually sits at 89.9%. Our blended OTP rose +0.73pp year-over-year while ship-to-home dropped -0.20pp over the same window — both series are in the mart for whoever's reconciling the blended number.

**gabriel.stroud:** On the cost side, our DC automation Phase 1 rollout at FON2 (Fontana) and JOL1 (Joliet) back in January-February 2026 lines up with the steepest drop in the cost-per-order series. Our cost per order has dropped from a peak of $8.60 during Q4 holiday strain down to **$7.30** in Q2 QTD. Q4 was also the only quarter with a QoQ cost *increase* (peak labor and the JOL1 storm), so some of the following drop is ordinary reversion sitting in the same window as the automation rollout — the two aren't separable at quarterly grain from this mart alone.

As for Leo's "Wider Promise Window" experiment (`exp_1187`, Jan 12 to Feb 20), we killed it on March 2. It showed a +4.2pp OTP bump on paper, but it ran at the same nodes and same window as the DC automation rollout, and it also carried a -0.6% conversion penalty from shoppers seeing longer delivery estimates. Leo's team has the deconfounding writeup if anyone wants the isolated read; net call was negative, so it's dead and buried.
...
On the benefit side, our "Benefit Onboarding Carousel" experiment (`exp_2556`, May 1 to June 15) wrapped up with a **+9pp lift in 30-day benefit awareness**. Our member data shows renewal by benefit-adoption depth: members using 2+ benefits renew at **95%**, compared to 71% for free-shipping-only members.

Streaming bundle users renew at **93%** on their own, and separately, only **34%** of our members report knowing they have access to it. We switched our streaming partner vendor from Vidora to Reelstream on June 1, 2026. Whether an awareness push is the right next move relative to our other CLTV levers is worth a proper side-by-side comparison rather than deciding it here. (And for the record, our proposed "Customer Lifetime Health Score" or CLHS remains firmly parked as a draft spec with no production table, so please don't cite a CLHS score in any formal reports).
